...
**Windows guidance (this splits the room — get it right):**
...
(Unified memory — the GPU sees all the RAM.) If they can't answer, explain it — it's
on the presenters' slides too.
...
1. **Ollama + API** (`notebooks/ollama_openai_api.ipynb`): after basic chat, have them
   attempt one structured-output call — messy text in, valid JSON out. Let them write
...
- TTS unbearably slow → CPU-only; `--engine piper` or `--text-only`.
