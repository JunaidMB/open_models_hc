...
def write_feed() -> None:
    ...
    for wav in sorted(PODCAST_DIR.glob("*.wav"), reverse=True):
        ...
        items.append(
            ...
            f"<enclosure url='{wav.name}' type='audio/wav' length='{wav.stat().st_size}'/>"
            f"<guid>{wav.name}</guid><pubDate>{format_datetime(pub)}</pubDate></item>"
        )
    feed = (
        "<?xml version='1.0' encoding='UTF-8'?><rss version='2.0'><channel>"
        "<title>My Private Podcast</title><description>Generated entirely on my own machine"
        "</description><link>http://localhost:8000</link>" + "".join(items) + "</channel></rss>"
    )
...
def main() -> None:
    ...
    print(f"Rewriting for audio with {LLM}...")
    ...
    print("Subscribe at:  http://<your-lan-ip>:8000/feed.xml")
